...
void SC4Core::SetTileType(WORD tileNum, WORD type, int levelNum) {
	...
	if (expandedROM) {
		*(rom + snes2pc(0xA28000) + (l % 0x20) * 0x400 + (l / 0x20) * 0x8000 + tileNum) = (type - 0xC8) / 2;
	}
}
...
void SC4Core::GenerateExpandedOffset() {
	...
	if (type == 0) {
		...
		addrList.push_back(region == 0 ? 0x81B4AC : 0x81B480); // simon
	}
	else if (type == 1) {
		for (unsigned i = 0; i < numLevels; i++) {
			DWORD addr;
...
void SC4Core::LoadTilesAndPalettes()
{
	...
	if (type == 0x0) {
		...
		palCache[0] = Get16Color(SNESCore::snes2pc(0x8694F7 + (level * 2)));  // background colors to cach and back
		palCacheOffset[0] = SNESCore::snes2pc(0x8694F7 + (level *2)); 
	...
	//	addrList.push_back(0x8787EE);		// first level. TODO HUD ENEMY PALETTES 
		...
		//addrList.push_back((0x87 << 16) | *LPWORD(rom + SNESCore::snes2pc(0x81A478 + level * 2)));	// sprite main //	87839c 80dda5 no list load for sprite?
...
static WORD GetEventPositionHighBits(const EventInfo &event) {
	return WORD(((event.ypos >> 8) & 0xF0) | ((event.xpos >> 12) & 0x0F));
}

static WORD GetEventXPosition(WORD rawX, WORD positionHighBits) {
	return WORD((rawX & 0x0FFC) | ((positionHighBits & 0x0F) << 12));
}

static WORD GetEventYPosition(WORD rawY, WORD positionHighBits) {
	return WORD((rawY & 0x0FFC) | ((positionHighBits & 0xF0) << 8));
}
	...
	if (type == 0) {
		...
		while (*LPDWORD(pevent) != 0xFFFFFFFF) {
			...
			if (event.type == 0 || event.type == 2) {
				...
				event.xpos = GetEventXPosition(event.xpos, event.eventSubId);
				event.ypos = GetEventYPosition(event.ypos, event.eventSubId);
			}
		...
		if ((addr & 0xFFFF) != 0xFFFF) {
			...
			if (*LPWORD(gfxPtr) != 0xFFFF) {
				...
				while (*LPWORD(gfxPtr) != 0xFFFF) {
					...
					if (addrBytes == 3) {
						if (dstAddr < 0x7E0000 || dstAddr - 0x7E0000 >= sizeof(ram)) {
							break;
						}
			...
			for (int i = 0; i < 0x20; i++) {
				...
				for (unsigned j = 0; j < 0x38 + 0x8; j += 8, blockIndex += (type == 2 ? 0x8 : 0x10)) {
					...
					if (type == 0) {
						...
						blockMap = *LPWORD(ram + 0x5000 + blockIndex); // blockNum? Contra3 ??
...
bool SC4Core::AddEvent(unsigned num, EventInfo &event) {
	...
	if (ok) {
		if (type == 0) {
			if (event.type == EVENT_TYPE_CANDLE || event.type == EVENT_TYPE_OBJECT) {
				// find free tag
...
static std::set<unsigned> SaveSpriteTilesToGraphicsPacket(
	...
	while (*LPWORD(gfxPtr) != 0xFFFF) {
		...
		DWORD srcPc = SNESCore::snes2pc(srcAddr);
		WORD packedSize = *LPWORD(core.rom + srcPc);
		BYTE unpacked[0x10000];
...
void SC4Core::SaveSprites() {
	...
	if (level != numLevels && expandedOffset.count(numLevels)) {
		offsetGroups.push_back(numLevels);
	}
	...
	for (auto tile : spriteUpdate) {
		...
		if (spriteVramByteBase + (tile << 5) + sizeof(tileData) <= sizeof(vram)) {
		...
		for (auto tile : spriteUpdate) {
			...
			for (auto group : offsetGroups) {
				...
				for (auto &entry : expandedOffset[group]) {
					unsigned expandedDstAddr = entry.first;
					if (expandedDstAddr >= 0x10000) continue;
