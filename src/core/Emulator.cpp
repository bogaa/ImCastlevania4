...
static DWORD WINAPI ThreadProc(LPVOID param) {
	...
	SetEvent(p->h);
	int saveStateSize = 0;
	bool coreNeedsFullpath = false;
	std::string fullpathRomPath;

	const auto deleteFullpathRom = [&]() {
		if (!fullpathRomPath.empty()) {
			DeleteFileA(fullpathRomPath.c_str());
			fullpathRomPath.clear();
		}
	};

	const auto makeFullpathRom = [&](const void *data, size_t size) -> const char * {
		deleteFullpathRom();
		char tempPath[MAX_PATH + 1] = {};
		char tempFile[MAX_PATH + 1] = {};
		if (!GetTempPathA(MAX_PATH, tempPath) || !GetTempFileNameA(tempPath, "SC4", 0, tempFile)) {
			Emulator::Instance()->retroLoadError = "Could not create temporary ROM file for libretro core.";
			return nullptr;
		}
		...
		if (!out.write(static_cast<const char *>(data), static_cast<std::streamsize>(size))) {
			...
			Emulator::Instance()->retroLoadError = "Could not write temporary ROM file for libretro core.";
			return nullptr;
		}
		out.close();
		fullpathRomPath = tempFile;
		return fullpathRomPath.c_str();
	};
	...
	while (true) {
		...
		if (found) {
			...
			case Emulator::Message::LOADROM: {
				...
				auto previousState = Emulator::Instance()->snesState;
				Emulator::Instance()->snesState = Emulator::SnesState::BUSY;
				if (previousState > Emulator::SnesState::OFF) {
					retro_unload_game();
				}
				const char *romPath = nmmx.filePath;
				void *romData = (void*)msg.lParam;
				size_t romSize = msg.wParam;
				if (coreNeedsFullpath) {
					romPath = makeFullpathRom(romData, romSize);
					romData = nullptr;
					romSize = 0;
					if (!romPath) {
						Emulator::Instance()->snesState = Emulator::SnesState::INVALID;
				...
				retro_game_info gameInfo = { romPath, romData, romSize, NULL };
				if (!retro_load_game(&gameInfo)) {
					Emulator::Instance()->snesState = Emulator::SnesState::INVALID;
					Emulator::Instance()->retroLoadError = "libretro failed to load ROM.";
					PostThreadMessage(mainThreadId, (int)Emulator::Instance()->snesState, 0, 0);
					break;
				}
...
bool Emulator::WaitForSnesStateMessage(MSG &msg, DWORD timeoutMs, const char *timeoutText) {
	DWORD waitStart = GetTickCount();
	while (true) {
		if (PeekMessage(&msg, (HWND)-1, (int)Emulator::SnesState::FIRST, (int)Emulator::SnesState::LAST, PM_REMOVE)) {
			return true;
		}
		if (GetTickCount() - waitStart > timeoutMs) {
			retroLoadError = timeoutText;
			return false;
		}
		Sleep(10);
	}
...
bool Emulator::Init() {
	...
	if (!threadHandle || WaitForSingleObject(p->h, 2000) != WAIT_OBJECT_0) {
		CloseHandle(p->h);
		delete p;
		retroLoadError = "Internal emulator thread did not start.";
		return false;
	}
	CloseHandle(p->h);
	delete p;
	...
	MSG msg;
	if (!WaitForSnesStateMessage(msg, 2000, "Internal emulator did not answer init.")) {
		Terminate();
		return false;
	}
...
bool Emulator::LoadRom(LPBYTE rom, unsigned size) {
...
bool  Emulator::LoadLevel(int num) {
	emuState = EmuState::ON;
	PostThreadMessage(threadId, Message::LOADLEVEL, (WPARAM)num, (LPARAM)0);
	MSG msg;
	if (!WaitForSnesStateMessage(msg, 5000, "Internal emulator did not answer level load.")) {
		Terminate();
		return false;
	}
	if (msg.wParam == (int)SnesState::INVALID) {
...
void Emulator::GetFrameState(FrameState &s) {
	...
	s.A8 = frameState.A8;
	s.AA = frameState.AA;
	memcpy(s.oam, (const void *)frameState.oam, sizeof(s.oam));
	s.simonAnimationCounter = frameState.simonAnimationCounter;
	s.simonSubAnimationCounter = frameState.simonSubAnimationCounter;
	s.simonSpriteAttribute = frameState.simonSpriteAttribute;
}

bool Emulator::ReadWram(unsigned snesAddress, unsigned byteCount, unsigned &value) {
	value = 0;
	if (!wram || byteCount == 0 || byteCount > 4) {
		return false;
	}

	unsigned offset = snesAddress;
	if (snesAddress >= 0x7E0000 && snesAddress <= 0x7FFFFF) {
		offset = snesAddress - 0x7E0000;
	}
	if (offset >= 0x20000 || byteCount > 0x20000 - offset) {
		return false;
	}

	AcquireFromEmu();
	for (unsigned i = 0; i < byteCount; ++i) {
		value |= static_cast<unsigned>(wram[offset + i]) << (i * 8);
	}
	return true;
}
...
bool Emulator::Terminate() {
	// tell the Emulator::Instance() thread to terminate
	if (!threadHandle || threadId == DWORD(-1)) {
		emuState = EmuState::OFF;
		return true;
	}
	...
	DWORD waitResult = WaitForSingleObject(threadHandle, 2000);
...
void Emulator::audio_sample(int16_t left, int16_t right) {
	if (!dsb_b || !device.buffer || !device.latency || !device.rings) return;
...
void Emulator::VideoRefresh(const void *data, unsigned width, unsigned height, size_t pitch) {
	...
	if (nmmx.type == 0) {
		memcpy((void *)Emulator::Instance()->frameState.oam, Emulator::Instance()->wram + RAM_ADDR(0x7E0F00), sizeof(Emulator::Instance()->frameState.oam));
		Emulator::Instance()->frameState.simonAnimationCounter = *LPWORD(Emulator::Instance()->wram + RAM_ADDR(0x7E0560));
		Emulator::Instance()->frameState.simonSubAnimationCounter = *LPWORD(Emulator::Instance()->wram + RAM_ADDR(0x7E0562));
		Emulator::Instance()->frameState.simonSpriteAttribute = *LPWORD(Emulator::Instance()->wram + RAM_ADDR(0x7E0566));
	}
